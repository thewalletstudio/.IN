...
document.addEventListener("click", e => {
  const orderButton = e.target.closest(".order-link[data-design]");

  if (orderButton) {
    e.preventDefault();
    const design = String(orderButton.dataset.design || "01").padStart(2, "0");
    window.location.href = `product.html?design=${encodeURIComponent(design)}`;
    return;
  }
