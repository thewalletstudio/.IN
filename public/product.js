...
if (!canvas) alert('DEBUG: canvas#preview not found in this page');
  ...
  try {
    if (side === 'front') {
      if (photoData) {
        ...
        if (!debugAlerted) {
          debugAlerted = true;
          alert('DEBUG: drew image ' + img.src + ' natural=' + img.naturalWidth + 'x' + img.naturalHeight);
        }
      }
    } else {
      let img;
