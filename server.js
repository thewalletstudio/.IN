...
/* Migration: add back_image to existing installs that were created before this column existed. */
try { db.exec("ALTER TABLE products ADD COLUMN back_image TEXT"); } catch (e) { /* column already exists */ }
...
/* Make existing design images at the repository root available to the new product page. */
app.use('/assets/products', express.static(ROOT, {index:false}));

/* Admin panel — explicit routes FIRST so /admin never gets swallowed by the root website. */
...
app.use('/admin', express.static(ADMIN, {index:false}));

/* Customer store pages — explicit routes FIRST, same reasoning as /admin above,
   so these always come from public/ (the maintained copies) and never get
   shadowed by the older duplicate files that still sit at the repository root
   (kept there only for the separate static GitHub Pages deployment).
   no-store headers so phones/Safari never serve a stale cached copy after a deploy. */
function sendFresh(res, filePath) {
  res.set('Cache-Control', 'no-store, no-cache, must-revalidate');
  res.sendFile(filePath);
}
app.get('/product.html', (req, res) => {
  sendFresh(res, path.join(PUBLIC, 'product.html'));
});
app.get('/checkout.html', (req, res) => {
  sendFresh(res, path.join(PUBLIC, 'checkout.html'));
});
app.get('/track.html', (req, res) => {
  sendFresh(res, path.join(PUBLIC, 'track.html'));
});
app.get('/product.js', (req, res) => {
  sendFresh(res, path.join(PUBLIC, 'product.js'));
});
app.get('/store.css', (req, res) => {
  sendFresh(res, path.join(PUBLIC, 'store.css'));
});
app.get('/checkout.js', (req, res) => {
  sendFresh(res, path.join(PUBLIC, 'checkout.js'));
});

/* Serve the EXISTING GitHub website from the repository root. */
...
app.use('/new', express.static(PUBLIC, {index:false}));
...
const productUpload = upload.fields([{name:'image',maxCount:1},{name:'backImage',maxCount:1}]);

app.post('/api/admin/products',auth,productUpload,(req,res)=>{
  const {name='Design',price=200,description=defaultDescription,category='photo'}=req.body;
  const image=req.files?.image?.[0]?'/uploads/'+req.files.image[0].filename:(req.body.image||'');
  const backImage=req.files?.backImage?.[0]?'/uploads/'+req.files.backImage[0].filename:(req.body.backImage||null);
  const r=db.prepare('INSERT INTO products(name,price,description,image,back_image,category,active) VALUES(?,?,?,?,?,?,1)').run(name,Number(price)||200,description,image,backImage,category);
  res.json({ok:true,id:r.lastInsertRowid});
});
app.put('/api/admin/products/:id',auth,productUpload,(req,res)=>{
  const old=db.prepare('SELECT * FROM products WHERE id=?').get(req.params.id); if(!old)return res.status(404).json({error:'Not found'});
  const image=req.files?.image?.[0]?'/uploads/'+req.files.image[0].filename:(req.body.image||old.image);
  const backImage=req.files?.backImage?.[0]?'/uploads/'+req.files.backImage[0].filename:(req.body.backImage||old.back_image);
  db.prepare('UPDATE products SET name=?,price=?,description=?,image=?,back_image=?,category=?,active=? WHERE id=?').run(req.body.name||old.name,Number(req.body.price)||old.price,req.body.description??old.description,image,backImage,req.body.category||old.category,req.body.active===undefined?old.active:Number(req.body.active),req.params.id);
  res.json({ok:true});
});
